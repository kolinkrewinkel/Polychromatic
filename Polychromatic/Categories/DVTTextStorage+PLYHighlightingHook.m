...
- (NSColor *)ply_colorAtCharacterIndex:(unsigned long long)index effectiveRange:(NSRangePointer)effectiveRange context:(NSDictionary *)context
{
    ...
    {
        long long nodeType = [self nodeTypeAtCharacterIndex:index effectiveRange:effectiveRange context:context];
        if (nodeType == 9)
        {
            id nameRanges = nil;
            id name = [self symbolNameAtCharacterIndex:index nameRanges:&nameRanges];
            
            color = [[PLYVariableManager sharedManager] colorForVariable:name inWorkspace:workspace];
        }
    }
    else
    {
